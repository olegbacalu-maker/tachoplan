...
- Live disruption per truck, labelled on the row for what it is — `TRAFFIC` in amber, `RAMP` in blue, because the two are not the same thing: **traffic delay** counts as driving — it burns hours and can force another break — while a **ramp queue** counts as other work and only shifts arrival. Both go in where the truck actually is — after the last point stamped as arrived, or in front of the whole run if it has not reached its first one — so every stop still ahead moves with them and is re-checked against its own booked slot. A tour already stamped to its end has taken the delay, and there it stays a tail
...
- **Measure with Google Maps** — optional, and never automatic. Give the tour the yard it leaves from and a destination per leg, press the button, and Google returns the distance and driving time of every leg at once. Nothing is sent until that button is pressed; the key is the dispatcher's own, kept on the device beside the board and never written into a CSV or a backup; measured figures stay on the board with no signal. Google fills the time until a human disagrees with it — once a leg's minutes are corrected by hand, that figure stands and the tool's is only shown beside it
- **Lorry speed.** Google times for a car, and across the EU a lorry over 3.5 t is held to 80 km/h where a car runs 120–130, so the map's figure arrives too early and a slot buffer built on it flatters the plan. Give the fleet's real average and every leg is timed at that instead — but only where it is the *slower* of the two: a lorry is never quicker than the car, so on a city leg or in a jam Google's own figure still stands. Leave it empty and nothing changes
...
Google routes for cars, not for lorries: bridge heights, axle weights, HGV bans and truck-specific detours are not in the measured distances. It times for a car too — the lorry-speed setting floors that, but a floor is not a lorry routing engine. Treat them as a good first figure, not as a route a 40-tonne vehicle is known to fit through.
